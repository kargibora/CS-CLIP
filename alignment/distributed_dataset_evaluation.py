"""
Distributed dataset evaluation utilities for multi-GPU training.
Splits datasets across GPUs and collects results efficiently.
"""
...
class DistributedDatasetEvaluator:
    ...
    def _get_dataset_subset_list(self) -> List[tuple]:
        ...
        for dataset_name in self.base_evaluator.datasets:
            if dataset_name not in self.base_evaluator.dataset_subsets:
                logging.warning(f"⚠️ Unknown dataset {dataset_name}, skipping...")
                continue
            ...
            for keys, values in zip(all_keys, all_values):
                for key, value in zip(keys, values):
                    if key not in combined_results:
                        ...
                        logging.warning(f"⚠️ Duplicate key {key} found during gather, keeping first value")
    ...
    def _gather_visualization_samples(self):
        """
        Gather visualization samples from all ranks to rank 0.
        
        Each rank has sampled images from the datasets it evaluated.
        We need to collect all samples to rank 0 for visualization generation.
        """
        if not self.is_distributed:
            return  # Single GPU: samples already in viz_logger
        
        # Get my samples
        my_samples = self.base_evaluator.viz_logger.dataset_samples
        
        logging.info(f"📸 Rank {self.rank}: Gathered {len(my_samples)} dataset/subset samples locally")
        ...
        # Use object list gathering
        dist.gather_object(my_samples, all_samples, dst=0)
        
        # Combine samples on rank 0
        if self.rank == 0:
            combined_samples = {}
            total_items = 0
            for rank_samples in all_samples:
                for key, samples in rank_samples.items():
                    if key not in combined_samples:
                        combined_samples[key] = samples
                        total_items += len(samples)
                    else:
                        # Merge samples from multiple ranks (shouldn't happen with proper split)
                        logging.warning(f"⚠️ Duplicate samples for {key} found during gather, merging")
                        combined_samples[key].extend(samples)
                        total_items += len(samples)
            
            # Replace viz_logger samples with combined samples
            self.base_evaluator.viz_logger.dataset_samples = combined_samples
            
            logging.info(f"✅ Gathered {len(combined_samples)} dataset/subset pairs with {total_items} total samples from {self.world_size} GPUs")
    ...
    def evaluate_all(
        ...
        if not dataset_subset_list:
            logging.warning(f"⚠️ Rank {self.rank}: No datasets to evaluate")
            return {}
        ...
        # Step 4.5: Gather visualization samples from all ranks to rank 0
        logging.info(f"📸 Rank {self.rank}: Gathering visualization samples to rank 0...")
        self._gather_visualization_samples()
        
        # Step 5: Generate visualizations (rank 0 only, after gathering samples)
        if self.rank == 0:
            num_sampled = len(self.base_evaluator.viz_logger.dataset_samples)
            if num_sampled > 0:
                logging.info(f"📸 Generating benchmark visualizations for {num_sampled} dataset/subset pairs...")
                logging.info(f"   Sampled datasets: {list(self.base_evaluator.viz_logger.dataset_samples.keys())}")
                try:
                    # Generate visualizations (returns dict with images and metrics)
                    viz_results = self.base_evaluator.viz_logger.log_all_datasets(
                        model=model,
                        clip_model=clip_model,
                    ...
                    # Always log visualization images to wandb (even if wandb_log=False for metrics)
                    # Visualizations are important for debugging and monitoring
                    if viz_results and viz_results.get('images'):
                        try:
                            import wandb
                            wandb.log({
                                f"{viz_results['wandb_prefix']}/samples": viz_results['images']
                            }, step=step)
                            logging.info(f"✅ Generated and logged {len(viz_results['images'])} visualization images to wandb")
                        except ImportError:
                            logging.warning("⚠️ wandb not available, skipping image logging")
                        except Exception as e:
                            logging.error(f"❌ Failed to log images to wandb: {e}")
                    else:
                        if viz_results:
                            logging.info(f"✅ Generated {len(viz_results.get('images', []))} visualizations (no wandb available)")
                        else:
                            logging.warning("⚠️ Visualization generation returned empty results")
                except Exception as e:
                    logging.error(f"❌ Failed to generate visualizations: {e}")
                    import traceback
                    logging.error(f"📍 Visualization traceback:\n{traceback.format_exc()}")
            else:
                logging.warning("⚠️ No samples to visualize (dataset_samples is empty)")
        
        # Step 6: Save to CSV (rank 0 only)
        if self.rank == 0:
            ...
            if wandb_log:
                try:
                    ...
                    logging.info(f"✅ Logged {len(all_results)} results to wandb")
                except ImportError:
                    logging.warning("⚠️ wandb not available, skipping wandb logging")
