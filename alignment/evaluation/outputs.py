...
def is_tca_model(model: Any) -> bool:
    """Check if model is a TCA model (has TokenConditionedHead)."""
    base_model = model.module if hasattr(model, 'module') else model
    
    # Check if it has a head attribute and if that head is TokenConditionedHead
    if hasattr(base_model, 'head'):
        head = base_model.head
        head_class_name = head.__class__.__name__
        return head_class_name == 'TokenConditionedHead'
