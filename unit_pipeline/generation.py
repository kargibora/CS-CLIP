"""
Unified Entity & Relational Generation
Extracts entity units, their relationships, and generates negative variants.

Key features:
1. Extract positive entity units from captions
2. Generate negative entity variants (semantically different but similar category)
3. Extract relations between entities (if they exist)
4. Generate negative relations (reversed, swapped, or modified)

Output: Single unified JSON with positive/negative entities and relations
"""
...
def extract_entities_and_relations_batched(
    llm,
    captions: List[str],
    batch_size: int = 16
) -> List[Dict[str, Any]]:
    """Extract visual entity units and relations from captions."""
    
    system_message = """Extract visual entity units and relations. Return ONLY valid JSON:
{"entities": ["item1", "item2"], "relations": [{"subject": "item1", "relation_type": "is action", "object": "item2"}]}
...
3. NO overlapping entities - each distinct object once with full description
4. Include counting numbers: "three dogs", "five people"
5. Match input language exactly
6. Max 12 entities, 12 relations
...
3. SUBJECT AND OBJECT must be from the entity list (exact match)
...
{"entities": ["man", "red umbrella"], "relations": [{"subject": "man", "relation_type": "is holding", "object": "red umbrella"}]}

"Cat sitting on the wooden table"
{"entities": ["cat", "wooden table"], "relations": [{"subject": "cat", "relation_type": "is sitting on", "object": "wooden table"}]}

"The dog is larger than the cat"
{"entities": ["dog", "cat"], "relations": [{"subject": "dog", "relation_type": "is larger than", "object": "cat"}]}

"Woman with a child near the fountain"
{"entities": ["woman", "child", "fountain"], "relations": [{"subject": "woman", "relation_type": "is with", "object": "child"}, {"subject": "woman", "relation_type": "is near", "object": "fountain"}]}

"Three red apples in a blue bowl"
{"entities": ["three red apples", "blue bowl"], "relations": [{"subject": "three red apples", "relation_type": "are in", "object": "blue bowl"}]}

"Gato sobre la mesa"
{"entities": ["gato", "mesa"], "relations": [{"subject": "gato", "relation_type": "está sobre", "object": "mesa"}]}

"Bird flying above the trees"
{"entities": ["bird", "trees"], "relations": [{"subject": "bird", "relation_type": "is flying above", "object": "trees"}]}

"The coffee cup next to the laptop"
{"entities": ["coffee cup", "laptop"], "relations": [{"subject": "coffee cup", "relation_type": "is next to", "object": "laptop"}]}"""""

    # Process in batches
    all_results = []
    num_batches = math.ceil(len(captions) / batch_size)
    
    for batch_idx in tqdm(range(num_batches), desc="Extracting entities"):
        ...
        for i, output in enumerate(batch_outputs):
            if isinstance(output, dict):
                ...
                for entity in entities:
                    comp_str = str(entity).strip()
                    
                    if (comp_str.lower() not in ['x', 'y', 'string', '...', '___'] and
                        len(comp_str) >= 2):
                        valid_entities.append(comp_str)
    ...
    print(f"Extracted entities from {len(all_results)} captions:")
...
def generate_entity_negatives_batched(
    llm,
    entities_list: List[List[str]],
    original_captions: List[str],
    batch_size: int = 16,
    n_neg_per_entity: int = 2,
) -> List[Dict[str, Any]]:
    """Generate hard negative entity variants.
    
    Args:
        llm: Language model wrapper
        entities_list: List of entity lists per caption
        original_captions: Original captions for context
        batch_size: Batch size for generation
        n_neg_per_entity: Number of negatives per entity
        
    Returns:
        List of dicts with 'negative_entities'
    """
    
    system_message = """Generate entity negatives for vision-language training. Return ONLY valid JSON:
...
1. SAME CATEGORY: dog -> cat, not dog -> table
2. NO SYNONYMS: avoid bike -> bicycle, car -> automobile
3. NOT TOO SIMILAR: avoid boat -> canoe, cup -> mug, sofa -> couch
...
SKIP if entity is abstract text ("Sign up", "Call now") → return empty []"""



    # Initialize results
    caption_results = {}
    for caption_idx in range(len(entities_list)):
        caption_results[caption_idx] = {
            'original_caption': original_captions[caption_idx],
            'entities': entities_list[caption_idx],
            'negative_entities': {},
        }
    ...
    for caption_idx, entities in enumerate(entities_list):
        if not entities:
            continue
        for entity in entities:
            safe_entity = str(entity).replace("{", "{{").replace("}", "}}")
            safe_caption = original_captions[caption_idx].replace("{", "{{").replace("}", "}}")
            
            prompt = f"{safe_entity}\nContext: {safe_caption}"

            prompts.append(prompt)
            entity_indices.append((caption_idx, entity))
    
    if not prompts:
        return [caption_results[i] for i in sorted(caption_results.keys())]
    
    print(f"Generating hard negatives for {len(prompts)} entities...")
    ...
    for batch_idx in tqdm(range(num_batches), desc="Component negatives"):
        ...
        batch_outputs = llm.generate(
            ...
            max_new_tokens=400,
            temperature=0.3,
            use_entity_negative_scheme=True,
            repetition_penalty=1.0,
            system_message=system_message,
            stop=["\n\n", "Component:", "Context:"]
        )
    ...
    for i, output in enumerate(all_outputs):
        ...
        all_entities_in_caption = entities_list[caption_idx]
        ...
        if isinstance(output, dict):
            ...
            for var in negative_variants:
                ...
                if neg and isinstance(neg, str):
                    neg_lower = neg.lower()
                    orig_lower = original_entity.lower()
                    ...
                    def is_entity_copy(neg_str, entities):
                        """Check if negative is identical to another entity in the caption."""
                        neg_normalized = neg_str.lower().strip()
                        for entity in entities:
                            comp_normalized = entity.lower().strip()
                    ...
                    def contains_other_entity(neg_str, original, entities):
                        """Check if negative contains significant parts of other entities."""
                        neg_words = set(neg_str.lower().split())
                        
                        for entity in entities:
                            if entity == original:
                                continue
                            comp_words = set(entity.lower().split())
                    ...
                    if is_entity_copy(neg, all_entities_in_caption):
                        rejected_entity_copies += 1
                        continue
                    
                    if contains_other_entity(neg, original_entity, all_entities_in_caption):
                        rejected_entity_copies += 1
                        continue
                    ...
                    if is_synonym_or_too_similar(original_entity, neg):
                        rejected_synonyms += 1
                        continue
                    ...
                    is_valid = (
                        ...
                        not has_only_number_change(original_entity, neg)
                    )
        ...
        # Add to results
        caption_results[caption_idx]['negative_entities'][original_entity] = valid_variants
    ...
    print(f"Entity negatives: {successful_negs} with alternatives, {empty_negs} empty")
    print(f"Rejected: {rejected_low_quality} low quality, {rejected_synonyms} synonyms/too-similar, "
          f"{rejected_entity_copies} entity copies, {rejected_duplicates} duplicates")
    ...
    total_variants = sum(
        len(neg_list) 
        for r in results 
        for neg_list in r['negative_entities'].values()
    )
    
    print(f"Generated {total_variants} entity negative variants")
    return results
...
3. NO SYNONYMS: 
   Avoid "is near" -> "is close to" because it preserves meaning.
   Prefer "is near" -> "is far from".

4. MATCH SUBJECT/OBJECT: Use exact entity names from input
...
Input: "cat is sitting on mat" | Entities: [cat, mat]
...
Input: "man is holding umbrella" | Entities: [man, umbrella]
...
Input: "dog is with owner" | Entities: [dog, owner]
...
Input: "book is on table" | Entities: [book, table]
...
Input: "car is larger than bicycle" | Entities: [car, bicycle]
...
Input: "gato está sobre la mesa" | Entities: [gato, mesa]
{"relation_negatives": [{"original_relation": "está sobre", "original_subject": "gato", "original_object": "mesa", "negatives": [{"relation_type": "está debajo de", "change_type": "antonym"}, {"relation_type": "no está sobre", "change_type": "negation"}, {"swap_subject": "mesa", "swap_object": "gato", "change_type": "swap"}]}]}"""
    ...
    for idx, data in enumerate(relational_data):
        ...
        prompts.append(prompt)
        prompt_metadata.append((idx, relations[:8], entities))
    ...
    for batch_idx in tqdm(range(num_batches), desc="Relational negatives"):
        ...
        try:
            ...
            print(f"\nError in batch {batch_idx} (prompts {start_idx}-{end_idx}): {e}")
    ...
    for i, output in enumerate(all_outputs):
        original_idx, original_relations, original_entities = prompt_metadata[i]
            ...
            for rel in original_relations:
                ...
                for neg in negatives[:5]:  # Allow up to 5 negatives per relation
                    ...
                    if change_type == 'swap':
                        ...
                        if swap_subj in original_entities and swap_obj in original_entities:
