...
class CheckpointConfig:
    ...
    # CLOVE-specific: pretrained weights for base model (e.g., "openai")
...
class BatchCheckpointEvaluator:
    ...
    def _load_local_checkpoint(
        ...
        alignment_type = "FT"
        logging.info(f"  Loading as FT checkpoint")
    ...
    def _load_ft_checkpoint(
        ...
        # load_clip_model() returns a wrapper for evaluation convenience, but FT
        # checkpoints were saved from a pipeline whose backbone keys are rooted at
        # model.* rather than model.model.*. Unwrap once here to match that layout.
        pipeline_clip_model = clip_model.model if hasattr(clip_model, "model") else clip_model
        
        # Create pipeline
        model = CLIPEndToEndPipeline(
            model=pipeline_clip_model,
            head=head,
            image_layer_names=image_layer_names,
            text_layer_names=text_layer_names,
        ...
        if missing_keys:
            ...
            if len(missing_keys) > 50:
                logging.warning(f"    ... and {len(missing_keys) - 50} more missing keys")
        
        if unexpected_keys:
            logging.warning(f"  Unexpected keys in checkpoint: {len(unexpected_keys)} keys")
            for key in unexpected_keys[:50]:
                logging.warning(f"    unexpected: {key}")
            if len(unexpected_keys) > 50:
                logging.warning(f"    ... and {len(unexpected_keys) - 50} more unexpected keys")
    ...
    def _clean_state_dict(self, state_dict: dict) -> dict:
        ...
        - model.model.* (double wrapped pipeline backbone)
        
        For local FT checkpoints we must preserve the top-level pipeline prefixes:
        - model.* for the CLIP backbone inside CLIPEndToEndPipeline
        - head.* for the FT head
        """
        cleaned = {}
        ...
        for key, value in state_dict.items():
            ...
            # model.model.* can appear when the pipeline backbone is wrapped again.
            # Collapse it to model.* but keep the pipeline-level model prefix.
...
DEFAULT_CHECKPOINTS_TO_EVALUATE = [
    ...
    # CheckpointConfig(
    #     name="FSC-CLIP ViT-B/32 (COCO)",
    #     csv_filename="fsc_clip_coco_vitb32.csv",
    #     checkpoint_type="external",
    #     checkpoint_path="external_checkpoints/fsc-clip/coco_fsc-clip-ViT-B-32.pt",
    #     base_model="ViT-B/32",
    # ),
