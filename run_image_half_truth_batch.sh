#!/bin/bash
# =============================================================================
# Batch Image Half-Truth Vulnerability Experiment Runner
# =============================================================================
# This script reads checkpoints from eval_checkpoints.yaml and runs the
# image half-truth vulnerability experiment for each one.
#
# The image half-truth test evaluates whether models can correctly identify
# the target image when multiple images share some visual components.
#
# Usage:
#   ./run_image_half_truth_batch.sh                          # Run all checkpoints
#   ./run_image_half_truth_batch.sh --dry-run                # Show commands without running
#   ./run_image_half_truth_batch.sh --config other.yaml      # Use different config
#   ./run_image_half_truth_batch.sh --filter "CLOVE"         # Run only checkpoints matching filter
#   ./run_image_half_truth_batch.sh --num_gpus 4             # Run in parallel on 4 GPUs
#   ./run_image_half_truth_batch.sh --save_all_scores        # Save all similarity scores for analysis
# =============================================================================
...
MAX_DISTRACTORS=5
SEED=42
RESULTS_BASE_DIR="./results/image_half_truth"
DRY_RUN=false
FILTER=""
SAVE_IMAGES=true
NUM_SAVE_IMAGES=20
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        --help|-h)
            ...
            echo "  --dataset NAME        Dataset to use: coco or laion (default: coco)"
            echo "  --max_samples N       Maximum samples to generate (default: 500)"
            echo "  --results_dir DIR     Base directory for results (default: ./results/image_half_truth)"
            ...
            echo "  --min_distractors N   Minimum distractors per sample (default: 2)"
            echo "  --max_distractors N   Maximum distractors per sample (default: 5)"
            echo ""
            echo "Output Options:"
            echo "  --save_images         Save visualization images (default: true)"
            echo "  --no_save_images      Don't save visualization images"
            echo "  --save_all_scores     Save all similarity scores for analysis (default: true)"
            echo "  --no_save_all_scores  Don't save all scores"
            echo ""
...
echo "============================================================"
echo "Batch Image Half-Truth Vulnerability Experiment"
echo "============================================================"
echo "Config file:      $CONFIG_FILE"
echo "Dataset:          $DATASET"
echo "Max samples:      $MAX_SAMPLES"
echo "Distractors:      $MIN_DISTRACTORS - $MAX_DISTRACTORS"
echo "Results dir:      $RESULTS_BASE_DIR"
echo "Num GPUs:         $NUM_GPUS"
echo "Save images:      $SAVE_IMAGES"
echo "Save all scores:  $SAVE_ALL_SCORES"
...
export CONFIG_FILE DATASET COCO_JSON COCO_IMAGE_ROOT MAX_SAMPLES SEED
export RESULTS_BASE_DIR DRY_RUN FILTER USE_SLURM SLURM_PARTITION
export SLURM_GPUS SLURM_TIME SLURM_MEM SAVE_IMAGES NUM_SAVE_IMAGES
export LAION_DATA_ROOT LAION_JSON_ROOT TAR_RANGE NUM_GPUS
export MIN_DISTRACTORS MAX_DISTRACTORS SAVE_ALL_SCORES
...
max_samples = os.environ.get('MAX_SAMPLES', '500')
seed = os.environ.get('SEED', '42')
results_base_dir = os.environ.get('RESULTS_BASE_DIR', './results/image_half_truth')
...
num_save_images = os.environ.get('NUM_SAVE_IMAGES', '20')
save_all_scores = os.environ.get('SAVE_ALL_SCORES', 'true').lower() == 'true'

# Sampling options
min_distractors = os.environ.get('MIN_DISTRACTORS', '2')
max_distractors = os.environ.get('MAX_DISTRACTORS', '5')
...
def build_command(ckpt, idx):
    ...
    output_name = csv_filename.replace('.csv', '').replace(' ', '_')
    output_dir = os.path.join(results_base_dir, output_name)
    
    cmd = ["python", "experiments/image_half_truth_vulnerability.py"]
    ...
    cmd.extend(["--min_distractors", min_distractors])
    cmd.extend(["--max_distractors", max_distractors])
    cmd.extend(["--output_dir", output_dir])
    cmd.extend(["--seed", seed])
    
    if save_images:
        cmd.append("--save_images")
        cmd.extend(["--num_save_images", num_save_images])
    
    # Note: save_all_scores is now always enabled in the Python script
    # (save_all_scores() and save_scores_csv() are called automatically)
...
def run_single_checkpoint(args):
    ...
    print(f"[GPU {gpu_id}]    Command: {cmd_str[:120]}...")
    ...
    try:
        ...
        print(f"[GPU {gpu_id}]    Error: {stderr[:200]}")
...
if num_gpus > 1 and len(filtered_checkpoints) > 1 and not use_slurm:
    ...
    with ProcessPoolExecutor(max_workers=num_gpus) as executor:
        ...
        for future in as_completed(futures):
            ...
            all_output_dirs.append((name, output_dir))
            if success:
    ...
    for task_idx, (ckpt_idx, ckpt) in enumerate(filtered_checkpoints):
        ...
        all_output_dirs.append((result_name, output_dir))
...
for name, output_dir in all_output_dirs:
    results_file = os.path.join(output_dir, 'results.json')
    if os.path.exists(results_file):
        with open(results_file) as f:
            data = json.load(f)
        batch_summary['models'][name] = {
        ...
        batch_summary['models'][name] = {
            'output_dir': output_dir,
            'metrics': None,
            'status': 'failed' if name in [f[0] for f in failed] else 'pending',
        }

# Save batch summary
batch_summary_path = os.path.join(results_base_dir, 'batch_summary.json')
...
if successful and not dry_run:
    print("\n📊 To analyze results, open the Jupyter notebook:")
    print(f"   experiments/image_half_truth_analysis.ipynb")
    print(f"\n   Set RESULTS_BASE_DIR = Path('{results_base_dir}')")
    print(f"   Load batch_summary.json for model comparison")
